import type React from "react";

/**
 * The Axiom brand mark — a dynamic "A" formed by two sweeping strokes,
 * rendered in Axiom blue. Self-contained colours so the mark stays
 * on-brand on any background.
 */
const MARK_WIDTH = 48;
const MARK_HEIGHT = 44;

const STROKE_PRIMARY =
  "M23.3136 0.000172577C23.7656 -0.00605594 23.921 0.156142 24.1433 0.514231C24.5509 1.16966 24.9009 1.85882 25.2687 2.53651L27.1908 6.07788L33.0947 16.9308C31.8192 18.8636 30.2932 20.6278 28.5552 22.1798C28.2073 21.4364 27.6831 20.5047 27.2923 19.7644L24.7031 14.8787C24.3176 14.148 23.785 13.2011 23.4606 12.4684C23.0447 13.3426 22.3635 14.5124 21.894 15.378L18.6731 21.292L14.3638 29.2211C13.529 30.7588 12.6725 32.3915 11.801 33.9017C12.793 33.4631 14.4233 32.949 15.5187 32.5115C18.5383 31.3291 21.4343 29.8654 24.1669 28.1413C30.1592 24.3097 35.1209 18.924 36.4043 11.869C36.5267 11.1972 36.6041 10.5141 36.7469 9.84772C36.8285 10.4874 36.7935 11.4111 36.7809 12.0675C36.7396 12.4292 36.7265 12.7951 36.6857 13.1533C34.7223 30.3381 16.9084 39.3342 1.48403 43.0826C1.17902 43.1566 0.767969 43.2439 0.466573 43.1428C-0.578555 42.7936 0.413054 41.5168 0.734277 40.9317L2.16974 38.3147L7.1813 29.1154L17.4297 10.2815L20.8145 4.05739C21.4407 2.90164 22.0628 1.74064 22.6952 0.588359C22.8846 0.24334 22.9652 0.176358 23.3136 0.000172577Z";

const STROKE_SECONDARY =
  "M38.5368 18.7218C38.5949 18.8947 38.5834 19.556 38.586 19.7823C38.609 21.9011 38.3406 23.8773 37.9901 25.9588C38.7707 27.639 47.1256 42.3444 47.1282 42.8973C47.1288 43.0964 47.0131 43.2486 46.8729 43.3844C46.7144 43.5375 46.5099 43.5692 46.2964 43.585C45.5373 43.6422 44.7489 43.6024 43.9866 43.6013L39.6695 43.5957L25.7783 43.5973C24.0289 43.5901 22.2797 43.5916 20.5304 43.6018C21.0917 43.0913 22.3192 42.2122 22.9642 41.696C24.3464 40.5974 25.6774 39.4384 26.9529 38.2223C30.3702 38.1677 34.048 38.1973 37.4701 38.2361C37.1206 37.7139 36.4583 36.4309 36.1412 35.8504C35.2314 34.1544 34.3044 32.4671 33.3606 30.7885C35.4365 27.6017 37.1169 24.444 38.0497 20.761C38.214 20.1123 38.3223 19.4113 38.518 18.7809L38.5368 18.7218Z";

interface AxiomMarkProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  /** "brand" uses Axiom blue; "light" renders a lighter blue for dark backgrounds. */
  tone?: "brand" | "light";
}

/** Icon-only Axiom mark. */
export const AxiomMark = ({
  size = 24,
  tone = "brand",
  ...props
}: AxiomMarkProps) => {
  const fill = tone === "light" ? "#5EA0F2" : "#0F6EEF";
  return (
    <svg
      width={(size * MARK_WIDTH) / MARK_HEIGHT}
      height={size}
      viewBox={`0 0 ${MARK_WIDTH} ${MARK_HEIGHT}`}
      fill="none"
      role="img"
      aria-label="Axiom"
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <path d={STROKE_PRIMARY} fill={fill} />
      <path d={STROKE_SECONDARY} fill={fill} />
    </svg>
  );
};

AxiomMark.displayName = "AxiomMark";

interface AxiomLogoProps extends React.SVGProps<SVGSVGElement> {
  height?: number;
}

/** Full Axiom logo — brand mark followed by the "axiom" wordmark. */
export const AxiomLogo = ({ height = 20, ...props }: AxiomLogoProps) => (
  <svg
    width={(height * 196) / MARK_HEIGHT}
    height={height}
    viewBox={`0 0 196 ${MARK_HEIGHT}`}
    fill="none"
    role="img"
    aria-label="Axiom"
    xmlns="http://www.w3.org/2000/svg"
    {...props}
  >
    <path d={STROKE_PRIMARY} fill="#0F6EEF" />
    <path d={STROKE_SECONDARY} fill="#0F6EEF" />
    <text
      x="64"
      y="32"
      fontFamily="Inter, system-ui, sans-serif"
      fontSize="30"
      fontWeight="700"
      letterSpacing="-1.4"
      fill="currentColor"
    >
      axiom
    </text>
  </svg>
);

AxiomLogo.displayName = "AxiomLogo";
